...
public:
    ...
    const AVFrame &decode_frame()
    {
        if (avcodec_send_packet(context, pkt) < 0)
            throw CodecException("ASDF");
        if (avcodec_receive_frame(context, frame) < 0)
            throw CodecException("HJKL");
        return *frame;
    }
};
...
{
public:
    PyOpusDecoder(int frame_size, int sample_rate, int channels) : FRAME_SIZE(frame_size), SAMPLE_RATE(sample_rate), CHANNELS(channels)
    {
        int err;
